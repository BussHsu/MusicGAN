...
class TARGET_LSTM(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim, sequence_length, start_token, params):
        ...
        with tf.variable_scope('generator'):
            #param is loaded from 'save/target_params.pkl' param[0] is?
            a= params[0]
        ...
        # processed for batch
        # processed x is x transformed by the embedding
        with tf.device("/cpu:0"):
            # get the self.x row in the self.g_embeddings
            a=tf.nn.embedding_lookup(self.g_embeddings, self.x)
            self.processed_x = tf.transpose(a, perm=[1, 0, 2])  # seq_length x batch_size x emb_dim

        # initial states
        self.h0 = tf.zeros([self.batch_size, self.hidden_dim])
        self.h0 = tf.stack([self.h0, self.h0])  #(2, 64(batch), 32)
        ...
        def _g_recurrence(i, x_t, h_tm1, gen_o, gen_x):
            ...
            log_prob = tf.log(tf.nn.softmax(o_t))   # multinomial distribution of next token
            next_token = tf.cast(tf.reshape(tf.multinomial(log_prob, 1), [self.batch_size]), tf.int32) # draw next token from multinomial dist.
            x_tp1 = tf.nn.embedding_lookup(self.g_embeddings, next_token)  # batch x emb_dim,
            gen_o = gen_o.write(i, tf.reduce_sum(tf.multiply(tf.one_hot(next_token, self.num_emb, 1.0, 0.0),
                                                             tf.nn.softmax(o_t)), 1))  # [batch_size] , prob (The probability of the token)
            gen_x = gen_x.write(i, next_token)  # indices, batch_size (The idx of the token)
            return i + 1, x_tp1, h_t, gen_o, gen_x

        # build a 20 step LSTM
        ...
        ta_emb_x = tensor_array_ops.TensorArray(
            dtype=tf.float32, size=self.sequence_length)
        ta_emb_x = ta_emb_x.unstack(self.processed_x) # array with 20 (64(batch)*32) elements
        ...
        # pretraining loss - E[tlgy]
        self.pretrain_loss = -tf.reduce_sum(
                                            # x: (batch , seqence_len) => one hot matrix with (batch*seq_len = 64*20, num_emb = 5000)
    ...
    def create_recurrent_unit(self, params):
        ...
        def unit(x, hidden_memory_tm1):
            ...
            ret =tf.stack([current_hidden_state, c])
            return ret
